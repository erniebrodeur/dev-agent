...
- `troubleshoot` activates implicitly for concrete failures. A request to fix an undiagnosed failure authorizes diagnosis only. The skill reproduces the mismatch, tests competing hypotheses, reports confidence and uncertainty, proposes the discovered corrective slice, and waits for its approval without editing implementation files.

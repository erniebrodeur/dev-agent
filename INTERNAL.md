...
- `bootstrap-repo-with-agent` runs only through explicit `$bootstrap-repo-with-agent` invocation. It first explains that it will merge the repository-root `AGENTS.md` and copy every other Pilot skill into `.agents/skills/`, then confirms intent, proposes exact changes, and writes only after final approval. Those repository copies are project-owned forks and do not synchronize automatically.
...
- `git-status` provides one concise, read-only owner for explicitly requested Git-state inspection and the sole automatic Git-status checkpoint immediately before an authorized release. Activation, recovery, implementation, verification, and commit preparation do not invoke it.
...
- `release` owns requests to prepare or execute a version release. It requires an exact user-supplied version and never infers the increment. It requires all substantive changes to be committed first, stages only version changes for a dedicated `version bump` commit, and proposes the exact commit, tag, branch, remote, and push operation. A later approval permits that transaction after the final Git-status preflight. It pushes the branch and tag atomically when supported and distinguishes a triggered publication from a verified remote release.

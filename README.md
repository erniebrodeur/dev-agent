...
Project context recovery also activates implicitly for requests such as “where are we?”, “resume,” or “what is next?” It verifies an existing root `CURRENT_WORK.md` against repository evidence. `CURRENT_WORK.md` is a Pilot convention for a mutable project scratch pad rather than a standard development artifact or design authority. Without that file, recovery performs a deeper scan, explains the convention, and offers to create either tracked or ignored project memory after reporting its findings.

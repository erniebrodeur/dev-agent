...
Diagnose the failure rigorously, propose the narrowest supported correction, and wait. This workflow is diagnosis-only even when the original request asks for a fix. A request to fix an undiagnosed failure authorizes diagnosis only, not the correction discovered later.
...
2. Establish the intended behavior, observed behavior, environment, reproduction conditions, and relevant recent change. Determine how success depends on the problem: user acceptance for a subjective outcome or observable evidence for a testable failure. Treat recent changes as evidence to examine, not assumed causes.
...
6. Gather only the evidence needed to distinguish those hypotheses. Use relevant tests, logs, runtime inspection, screenshots, browser behavior, coverage, or disposable probes without modifying project implementation.
...
8. Propose the smallest coherent corrective slice, including its outcome, scope, constraints or invariants, non-goals, dependencies, and verification expectations. Make those expectations target the actual observed failure or the user acceptance needed for a subjective outcome. Do not implement it.
